...
class MotorController:
    ...
    def get_BatteryCurrent(self):
        motorCommandString1 = "?BA 1\r"
        motorCommandString2 = "?BA 2\r"
        self.MotorSerialObj.write(bytes(motorCommandString1, 'utf-8'))
        while self.MotorSerialObj.in_waiting <= 0 :
            pass
        while self.MotorSerialObj.in_waiting > 0 :
            line1 = self.MotorSerialObj.readline().decode('utf-8')
            self.MotorSerialObj.write(bytes(motorCommandString2, 'utf-8'))
        ...
        while self.MotorSerialObj.in_waiting > 0 :
            line2 = self.MotorSerialObj.readline().decode('utf-8')
            lines = [line1, line2]
        with open ('CurrentReadings.txt', 'a') as file:
            file.write(f"Motor 1 Current: {line1}; Motor 2 Current: {line2} \n")    

    def get_RPM(self):
        motorCommandString1 = "?S 1\r"
        motorCommandString2 = "?S 2\r"
        self.MotorSerialObj.write(bytes(motorCommandString1, 'utf-8'))
        while self.MotorSerialObj.in_waiting <= 0 :
            pass
        while self.MotorSerialObj.in_waiting > 0 :
